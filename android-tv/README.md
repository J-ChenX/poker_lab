...
Android TV 遥控器应用。启动后以 1920 × 1080 全屏显示与
`https://tv.example.com/display` 目标图等比对齐的原生看板。布局按 3840 × 2160 网页定稿缩放到电视输出，不再依赖旧电视 WebView 对现代 CSS/React 的支持。

## 功能

- 原生复刻定稿 `/display` 的关闭状态和 4:6 控制面板状态
...
1. 将 `PokerLab-TV-v2.5.0.apk` 复制到 U 盘。
