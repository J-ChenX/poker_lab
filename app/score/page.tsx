    ...
    <main className={styles.shell}>
      ...
      <section className={styles.dashboard}>
        <aside className={styles.roster}>
          ...
          <form className={styles.registerForm} onSubmit={addPlayer}><div className={styles.registerLabel}><span>添加人员</span><small>{hydrated ? `${players.length} 人` : "读取中"}</small></div><input aria-label="人员姓名或昵称" value={newName} onChange={(event) => setNewName(event.target.value)} maxLength={20} placeholder="输入姓名或昵称" /><button disabled={!newName.trim()}>＋ 添加</button></form>
