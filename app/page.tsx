  ...
  return (
    <main>
      ...
      <section className="leaders-section catalogue-section">
        <div className="leaders-intro live-distribution-intro"><p className="eyebrow">LIVE HAND DISTRIBUTION</p>{liveDistribution ? <><div className="live-mini-head"><div><span>九类牌型概率</span><strong>最常见：{liveDistribution.bestHand === "一对" ? "对子" : liveDistribution.bestHand}</strong></div><small>{liveDistribution.samples.toLocaleString()} 种公共牌结果</small></div><div className="live-probability-list">{HAND_NAMES.map((name, index) => ({ name, value: liveDistribution.categories[index] })).filter(({ value }) => value > 0).reverse().map(({ name, value }) => <div className="live-probability-row" key={name}><span>{name === "一对" ? "对子" : name}</span><div><i style={{ width: `${value}%` }} /></div><strong>{value.toFixed(1)}%</strong></div>)}</div><p className="live-note">每次选牌后自动穷举更新，无需开始多人胜率计算。</p></> : <div className="live-intro-empty"><span>3+</span><h2>九类牌型概率</h2><p>选择两张底牌与至少三张公共牌后实时出现。</p></div>}</div>
